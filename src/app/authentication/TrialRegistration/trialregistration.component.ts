...
export class TrialRegistrationComponent implements OnInit, AfterViewInit {
  ...
  /** See registration: slow geo response must not overwrite +1 → US with IP-based ZA. */
  ...
  onCountrySelected(event: any): void {
    ...
    const match = this.countryNumberCode.find((x) => {
      const rowId = x?.countryId ?? x?.CountryId;
      return (
        typeof rowId === 'string' &&
        typeof selected?.id === 'string' &&
        rowId.toLowerCase() === selected.id.toLowerCase()
      );
    });
