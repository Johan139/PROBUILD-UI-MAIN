...
activeByUserId?: Record<string, { subscriptionId: string; packageLabel?: string }>;
  constructor(
    ...
    public dialogRef: MatDialogRef<SubscriptionCreateComponent, SubscriptionSelection | undefined>,
...
ngOnInit(): void {
  const currentPkg = this.data.currentValue
    ? this.data.packages.find(p => p.value === this.data.currentValue) ?? null
    : null;
  ...
  this.form.get('forWhom')!.valueChanges.subscribe(val => {
    ...
    else { tm.clearValidators(); tm.setValue(null); }
...
save(): void {
  ...
  const assigneeUserId = forWhom === 'team' ? this.form.value.teamMemberId : this.data.userId;
...
hasActiveForAssignee(): boolean {
  const forWhom = this.form.value.forWhom;
  const targetId = forWhom === 'self' ? this.data.userId : this.form.value.teamMemberId;
