...
export class InvoiceUploadComponent {
  ...
  onFileSelected(event: Event): void {
    ...
    if (fileList && fileList[0]) {
      ...
      this.filePreviewUrl = this.sanitizer.bypassSecurityTrustResourceUrl(URL.createObjectURL(this.fileToUpload));
    } else {
  ...
  onSubmit(): void {
    if (this.uploadForm.valid && this.fileToUpload) {
      ...
      this.invoiceService.uploadInvoice(this.fileToUpload, this.jobId, amount).subscribe({
        next: (uploadProgress) => {
