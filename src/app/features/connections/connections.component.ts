...
export class ConnectionsComponent implements OnInit, AfterViewInit {
  ...
  applyFilter(event: Event): void {
    ...
    this.showInviteMessage =
      this.dataSource.filteredData.length === 0 && filterValue.length > 0;
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.invitationService.inviteUser(result).subscribe({
          ...
          error: (err) => {
            console.error(
              '[ConnectionsComponent] inviteUser call failed:',
              err,
            );
            this.snackBar.open(
              `Error: ${err.error?.message || 'An unknown error occurred'}`,
              'Close',
              { duration: 5000 },
            );
          },
  ...
  loadConnections(): void {
    ...
    this.connectionService
      .getConnections()
      .subscribe((connections: ConnectionDto[]) => {
  ...
  updateDataSource(): void {
    const displayUsers: DisplayUser[] = this.allUsers.map((u) => ({
      ...u,
      isInSystem: true,
    }));
    const invitedUsers: DisplayUser[] = this.invited.map(
      (i) =>
        ({
          id: i.id,
  ...
  isRequestSent(userId: string): boolean {
    return this.allConnections.some(
      (c) =>
        c.otherUserId === userId &&
        c.status === 'PENDING' &&
        c.requesterId === this.currentUserId,
    );
  }
  ...
  isRequestReceived(userId: string): boolean {
    return this.allConnections.some(
      (c) =>
        c.otherUserId === userId &&
        c.status === 'PENDING' &&
        c.receiverId === this.currentUserId,
    );
  }

  cancelConnectionRequest(userId: string): void {
    const connection = this.allConnections.find(
  ...
  getConnectionStatus(userId: string): string {
    ...
    if (connection) {
      if (connection.status === 'PENDING') {
        return connection.requesterId === this.currentUserId
          ? 'Request Sent'
          : 'Request Received';
      }
