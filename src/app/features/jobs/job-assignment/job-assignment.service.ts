...
export class JobAssignmentService {
  ...
  getJobAssignment(): Observable<JobAssignment[]> {
    ...
    return this.http
      .get<JobAssignment[]>(url, { headers: this.getHeaders() })
      .pipe(
        catchError((error) => {
  ...
  getAvailableUser(): Observable<JobUser[]> {
    ...
    return this.http.get<JobUser[]>(url, { headers: this.getHeaders() }).pipe(
      catchError((error) => {
        console.error('Error fetching profile:', error);
        return throwError(() => new Error('Failed to load profile'));
      }),
  ...
  createJobAssignment(
    assignmentData: JobAssignmentLink,
  ): Observable<JobAssignment> {
    return this.http
      .post<JobAssignment>(this.apiUrl, assignmentData, {
  ...
  deleteUserAssignment(assignmentLink: JobAssignmentLink): Observable<void> {
    ...
    return this.http
      .post<void>(url, assignmentLink, { headers: this.getHeaders() })
      .pipe(
        catchError((error) => {
