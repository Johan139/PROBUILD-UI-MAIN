...
export class JobAssignmentComponent implements OnInit {
  ...
  teamMembers: any[] = [];
  currentUser: any;
  newAssignment: { email: string; jobRole: string } = {
    email: '',
    jobRole: '',
  };
  ...
  ngOnInit(): void {
    ...
    this.jobRoles = userTypes.filter(
    ...
    this.authService.currentUser$
      .pipe(
        ...
        switchMap((currentUser) => {
          ...
          const allAssignmentsObservable =
            this.jobAssignmentService.getJobAssignment();
          const teamMembersObservable =
            this.teamManagementService.getTeamMembers(
  ...
  private flattenAssignments(
    assignments: JobAssignment[],
  ): { job: JobAssignment; user: JobUser }[] {
  ...
  submitAssignment(): void {
    ...
    this.jobAssignmentService.createJobAssignment(assignmentData).subscribe({
      next: (response: JobAssignment) => {
        ...
        const jobIndex = this.jobAssignmentList.findIndex(
        ...
        this.filterAssignments();
        this.snackBar.open('Job assignment created successfully', 'Close', {
          duration: 3000,
        });
        this.resetForm();
        this.isSaving = false;
      ...
      error: (error) => {
        ...
        this.snackBar.open('Failed to create job assignment', 'Close', {
          duration: 3000,
        });
        this.isSaving = false;
      },
  ...
  deleteUserAssignment(job: JobAssignment, user: JobUser): void {
    if (!job || !user) {
      this.snackBar.open('Invalid job assignment or user', 'Close', {
        duration: 3000,
      });
      return;
    }
    ...
    this.jobAssignmentService.deleteUserAssignment(assignmentLink).subscribe({
      next: () => {
        ...
        this.filterAssignments();
        this.snackBar.open('Job assignment deleted successfully', 'Close', {
          duration: 3000,
        });
        this.isLoading = false;
        this.cdr.detectChanges();
      ...
      error: (error) => {
        ...
        this.snackBar.open('Failed to delete job assignment', 'Close', {
          duration: 3000,
        });
        this.isLoading = false;
      },
  ...
  onJobSelectionChange(job: JobAssignment): void {
    ...
    if (job && job.id !== 0) {
      ...
      this.availableUsers = this.userList.filter(
        (user) => !assignedUserIds.includes(user.id),
      );

      if (this.selectedUser && assignedUserIds.includes(this.selectedUser.id)) {
        this.snackBar.open(
          'This user is already assigned to the selected job.',
          'Close',
          { duration: 3000 },
        );
        this.selectedUser = null;
  ...
  private _filterJobs(value: string | JobAssignment): JobAssignment[] {
    ...
    const filtered = this.jobsForDropdown.filter((job) =>
      (job.projectName || '').toLowerCase().includes(filterValue),
    );
    return [{ id: 0, projectName: '', jobUser: [] }, ...filtered];
  }
  ...
  private _filterUsers(value: string | JobUser): JobUser[] {
    const filterValue =
      typeof value === 'string'
        ? value.toLowerCase()
        : (value ? `${value.firstName} ${value.lastName}` : '').toLowerCase();
