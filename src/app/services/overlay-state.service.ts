...
export class OverlayStateService {
  private readonly _blueprintData =
    new BehaviorSubject<BlueprintAnalysisData | null>(null);
  public readonly blueprintData$: Observable<BlueprintAnalysisData | null> =
    this._blueprintData.asObservable();

  private readonly _isOverlayVisible = new BehaviorSubject<boolean>(true);
  public readonly isOverlayVisible$: Observable<boolean> =
    this._isOverlayVisible.asObservable();

  private readonly _selectedElement = new BehaviorSubject<SelectableElement>(
    null,
  );
  public readonly selectedElement$: Observable<SelectableElement> =
    this._selectedElement.asObservable();
  ...
  private readonly _cursorPosition = new BehaviorSubject<{
    x: number;
    y: number;
  }>({ x: 0, y: 0 });
  public readonly cursorPosition$: Observable<{ x: number; y: number }> =
    this._cursorPosition.asObservable();

  setBlueprintData(data: BlueprintAnalysisData | null): void {
    this._blueprintData.next(data);
  }
  toggleOverlayVisibility(): void {
    this._isOverlayVisible.next(!this._isOverlayVisible.value);
  }
  setOverlayVisibility(isVisible: boolean): void {
    this._isOverlayVisible.next(isVisible);
  }
